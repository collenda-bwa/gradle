...
`org.gradle.hideWelcomeMessage=(true,false)`::
When set to `true`, Gradle won't generate welcome message.
_Disabled by default and welcome message is shown on first execution of each new version of Gradle._
